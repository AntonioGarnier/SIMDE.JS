...
export class Superescalar extends Machine {
    ...
    ticDecoder() {
        for (let i = this.prefetchUnit.first; !this.decoder.isFull() && i !== this.prefetchUnit.end(); i = this.prefetchUnit.nextIterator(i)) {
            let aux = this.prefetchUnit.elements[i];
    ...
    checkRegister(register, floatingPoint, reserveStationEntry, j) {
        let q = -1;
        let v = 0;
        if (floatingPoint) {
    ...
    issueInstruction(instruction, type, robIndex) {
    ...
    ticIssue() {
        ...
        for (let i = this.decoder.first; i !== this.decoder.end(); i = this.decoder.nextIterator(i), cont++) {
            let instruction = this.decoder.elements[i].instruction;
            if (this.reorderBuffer.isFull()) {
                break;
            }
            let fuType = Parser.opcodeToFunctionalUnitType(instruction.opcode);
            if (this.reserveStationEntry[fuType].length === this.getReserveStationSize(fuType)) {
                break;
            }
            let newROB = new ReorderBufferEntry();
            newROB.value = 0.0;
            newROB.destinyRegister = -1;
            newROB.address = -1;
            let robPos = this.reorderBuffer.add(newROB);
            let newER = new ReserveStationEntry();
    ...
    writeInstruction(type, num) {
        let resul;
        let inst = this.functionalUnit[type][num].getTopInstruction();
        if (inst != null) {
            let i = 0;
    ...
    ticCommit() {
        for (let i = 0; i < this.issue; i++) {
            if (this.reorderBuffer.isEmpty()) {
                ...
                let aux = this.reorderBuffer.remove();
                /* tslint:disable ter-indent */
    ...
    changePrediction(address, result) {
