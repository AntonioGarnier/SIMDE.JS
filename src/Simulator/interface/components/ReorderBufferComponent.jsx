...
class ReorderBufferComponent extends React.Component {
    ...
    handleClick(instructionId, instructionColor) {
        this.setState(Object.assign({}, this.state, { displayColorPicker: true, instructionId: instructionId, selectedColor: instructionColor }));
    }
    onColorAccept(value) {
        SuperescalarIntegration.colorCell(this.state.instructionId, this.state.selectedColor);
        this.setState(Object.assign({}, this.state, { displayColorPicker: false, selectedColor: '', instructionId: null }));
    }
    onColorCancel() {
        this.setState(Object.assign({}, this.state, { displayColorPicker: false, selectedColor: '', instructionId: null }));
    }
    ...
    render() {
        ...
        return (<div className="smd-reorder_buffer panel panel-default reorder-zone">
            ...
            this.props.content
                ...
                .map(e => (<div className="smd-table_row smd-reorder_buffer_entry" style={{ background: e.row.instruction.color }} onClick={() => this.handleClick(e.row.instruction.id, e.row.instruction.color)} title={e.row.instruction.value} key={'ReorderBuffer' + e.i}>
